...
bab_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern bab_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern bab_intg line; /* source code line number - defined in scanner.c */
...
extern bab_void startParser(bab_void);
extern bab_intg startScanner(ReaderPointer sc_buf);

static bab_void printParserError(bab_char* fmt, ...);
static bab_void displayParser(ReaderPointer ptrBuffer);
static bab_long getParserFileSize(bab_char* fname);
static bab_void callGarbageCollector(bab_void);
...
bab_void printParserError(bab_char* fmt, ...) {
...
bab_long getParserFileSize(bab_char* fname) {
	FILE* input;
	bab_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
bab_void displayParser(ReaderPointer ptrBuffer) {
...
bab_void callGarbageCollector(bab_void) {
