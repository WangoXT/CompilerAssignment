...
bab_intg line;								/* Current line number of the source code */
extern bab_intg errorNumber;				/* Defined in platy_st.c - run-time error number */

extern bab_intg stateType[];
extern bab_char* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern bab_intg transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(void) {
	...
	bab_char c;	/* input symbol */
	bab_intg state = 0;		/* initial state of the FSM */
	bab_intg lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	bab_intg lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	bab_intg lexLength;		/* token length */
	bab_intg i;				/* counter */
	bab_char newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((bab_intg)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
bab_intg nextState(bab_intg state, bab_char c) {
	bab_intg col;
	bab_intg next;
	col = nextClass(c);
...
//			currentToken.attribute.intValue = (bab_intg)tlong;
//		}
//		else {
//			currentToken = (*finalStateTable[ESNR])(lexeme);
//		}
...
Token funcSL(bab_char lexeme[]) {
	Token currentToken = { 0 };
	bab_intg i = 0, len = (bab_intg)strlen(lexeme);
...
bab_void printToken(Token t) {
	extern bab_char* keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
	...
	case STR_T:
		...
		printf("%s\n", readerGetContent(stringLiteralTable, (bab_intg)t.attribute.codeType));
		break;
	case KW_T:
